...
if selected_exp:
    ...
    show_relative = True #st.checkbox("显示加速比（以im2col为基准）", key="latency_relative")
    ...
    if show_relative:
        ...
        plot_df['latency'] = plot_df.apply(
            lambda x: baseline[x['op']] / x['latency'],  # Changed division order for speedup
            axis=1
        )
    ...
    energy_plot_df['energy'] = energy_plot_df.apply(
        lambda x: x['energy'] / energy_baseline[x['op']],
        axis=1
    )
