...
from baselines import SDK, im2col, vw_sdk
...
def polycim(cim_row, cim_col, operator):
    ...
    cim_cfg = CIMConfig(
        n_row=1, n_group_vcol=cim_col, n_comp=cim_row, n_group=1, n_macro=1
    )
    flops = operator.domain.count_val().get_num_si()
    op, cim_flops = run_pipeline(
...
def compare_conv2d(cim_row, cim_col, oh, ow, oc, ic, kh, kw, dilation=1):
    ...
    operator = benchmark.get_op_conv2d(
        b=1, oc=oc, ic=ic, oh=oh, ow=ow, kh=kh, kw=kw, stride=1, virtual_axis=True
    )
    ...
    polycim_is_best = (
        not_worse_than_im2col and not_worse_than_sdk and not_worse_than_vwsdk
    )
...
def compare_conv3d(cim_row, cim_col, oh, ow, oz, kh, kw, kz, dilation=1):
    ...
    operator = benchmark.get_op_dwconv3d(
        ic=1, ox=oh, oy=ow, oz=oz, kx=kh, ky=kw, kz=kz, stride=1, virtual_axis=True
    )
    # polycim_cycle = polycim(cim_row, cim_col, operator)[0]
    # polycim_cycle = int(polycim_cycle)
    polycim_cycle = 0
...
def compare_conv1d_45degree(cim_row, cim_col, oh, ow, oc, ic, kh):
    ...
    operator = benchmark.get_op_conv1d(
        oc=oc, ic=ic, oh=oh, ow=ow, k=kh, virtual_axis=True
    )
    polycim_cycle = polycim(cim_row, cim_col, operator)[0]
    polycim_cycle = int(polycim_cycle)
    ...
    with open("compare_conv2d_for_convnext.json", "w") as f:
        for cim_row, cim_col in cim_config_list:
            for op_args in op_config_list:
                result = compare_conv2d(cim_row=cim_row, cim_col=cim_col, **op_args)
                json.dump(result, f)
                f.write("\n")
                f.flush()
