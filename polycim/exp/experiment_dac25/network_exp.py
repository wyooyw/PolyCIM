...
def run_model(json_model_path, result_json_path, skew):
    ...
    run_op_info_key_to_index = {
        parse_op_info_to_key(op_info): idx
        for idx, op_info in enumerate(run_op_info_list)
    }
...
if __name__ == "__main__":
    for model in ["convnext_tiny"]:  # , "RepLKNet-31B", "SLaK_tiny"]:
        ...
        run_model(
            f"models/json/{model}.json",
            os.path.join(result_dir, "skew.json"),
            skew=True,
        )
        run_model(
            f"models/json/{model}.json",
            os.path.join(result_dir, "im2col.json"),
            skew=False,
        )
