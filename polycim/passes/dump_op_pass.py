...
class DumpOpPass(BreadthFirstPass):
    ...
    def dump_op(self, op):
        ...
        # save schedule code
        dump_code = dump_schedules(origin_op, op, min_compute_times=min_compute_times, cim_cfg=self.cim_config, flops=flops)
        with open(os.path.join(save_dir_solution, f"schedule_code.py"), "w") as f:
            f.write(dump_code)
        
        # save mapping pictures
        for idx, value in enumerate(extract_frame_info(op, self.cim_config, different_weight=True)):
