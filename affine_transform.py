...
def find_base_with_max_reuse(
    n_dim, dim_sizes, max_reuse_factor, hyperplanes, exclude_null_space_of=None
):
    global second_array
    """
...
def find_bases_with_max_reuse(
    ...
    for base in foreach_nontrival_point(bases):
        search_status = SearchStatus(
            bases=pre_bases.col_join(Matrix([base])), max_reuse=reuse
        )
...
def main():
    ...
    access_I = isl.BasicMap(
        "{ S[i0, i1, i2, i3, i4] -> I[o0, o1] : o0 = 2i0 + i1 + i3 and o1 = i2 + i4}"
    )
    access_O = isl.BasicMap(
        "{ S[i0, i1, i2, i3, i4] -> O[o0, o1] : o0 = 2i0 + i1 and o1 = i2}"
    )
    # print(f"{domain=}")
    # print(f"{access_I=}")
    # print(f"{access_O=}")
