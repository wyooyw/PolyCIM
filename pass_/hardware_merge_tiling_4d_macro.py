...
def get_macro_4d_hardware_tiling_schedule(op, software_schedule, cim_cfg):
    ...
    merge_range_iters = keep_iters + [
        "time",
        "row",
        "comp",
        f"ogroup * {n_igroup} + igroup",
        "col",
    ]
...
def macro_4d_hardware_tiling(op, all_schedule, cim_cfg):
    new_schedules = []
    for schedule in all_schedule:
        tile_schedule, merge_group_schedule = get_macro_4d_hardware_tiling_schedule(
            op, schedule, cim_cfg
        )
        # schedule = schedule.apply_range(hardware_tiling_schedule)
...
def macro_5d_hardware_tiling(op, all_schedule, cim_cfg):
    new_schedules = []
    for schedule in all_schedule:
        tile_schedule, reorder_schedule, n_use_group = (
            get_macro_5d_hardware_tiling_schedule(op, schedule, cim_cfg)
        )
        # schedule = schedule.apply_range(hardware_tiling_schedule)
...
def hardware_merge_tiling_pass(op_list, cim_cfg=None):
    ...
    for op in tqdm(op_list):
        ...
        for idx, (
            ...
            assert (
                sizes[-5] <= cim_cfg.n_row
            ), f"{sizes=}. {sizes[-4]=} should be less than {cim_cfg.n_row=}"
            assert (
                sizes[-4] <= cim_cfg.n_comp
            ), f"{sizes=}. {sizes[-3]=} should be less than {cim_cfg.n_comp=}"
            assert (
                sizes[-3] * sizes[-2] <= cim_cfg.n_group
            ), f"{sizes=}. {sizes[-2]=} should be less than {cim_cfg.n_group=}"
            assert (
                sizes[-1] <= cim_cfg.n_group_vcol
