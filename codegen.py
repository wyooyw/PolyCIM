...
class CodeGenerator:
    ...
    def codegen_special_defs(self, depth):
        with open("/home/wangyiou/project/cim_compiler_frontend/playground/op/bit_sparse/bit_sparse_conv2d_group/lib/def_special_regs.cim", "r") as f:
    ...
    def codegen_aff(self, aff, arg_list, depth):
        ...
        domain_iter_names = [aff.get_dim_name(isl.dim_type.in_, i) for i in range(n_domain_iter)]
        coef_list = []
    ...
    def _codegen_expr_max(self, arg_list, depth):
        ...
        code = CodeStmt(
            code=f"{new_var} = Max({arg_list[0]}, {arg_list[1]});",
            depth=depth
        )
        return code, new_var
    ...
    def _codegen_expr_min(self, arg_list, depth):
        ...
        new_var = alloc_unique_var()
        code = CodeStmt(
            code=f"{new_var} = Min({arg_list[0]}, {arg_list[1]});",
            depth=depth
        )
    ...
    def codegen_expr_binary(self, binary_op, arg_list, depth):
        ...
        new_var = alloc_unique_var()
        code = CodeStmt(
            code=f"{new_var} = {arg_list[0]} {binary_op} {arg_list[1]};",
            depth=depth
        )
        return [code], new_var
    ...
    def codegen_call(self, expr, depth):
        ...
        if type(op)==DataMovement:
            ...
            call_code = CodeStmt(
                code=f"{call_name}({call_args_str})",
                depth=depth
            )
            call_code = [call_code]
    ...
    def codegen_call_data_movement(self, op, call_args, depth):
        ...
        trans_code = CodeStmt(
            code=f"Trans({slice_var_I}, {slice_var_O});",
            depth=depth
        )
    ...
    def codegen_tensor_access_from_pw_multi_aff(self, tensor_access, call_args, depth):
        ...
        code_list = []
        # parse offset
        offset_code_list, offset_var_list = self.codegen_access_indices(access_offset, depth)
    ...
    def codegen_access_indices(self, access_op, depth):
        assert access_op.get_type()==isl._isl.ast_expr_type.op, f"{access_op.get_type()=}"
...
def align_compute_and_assign_schedules(compute_schedule, assign_schedules, levels):
    ...
    for idx,level in enumerate(levels):
        ...
        for i in range(len(assign_schedules_at_level)):
            assign_schedules_at_level[i] = insert_const_dim_in_range(assign_schedules_at_level[i], level+idx, i)
        ...
        for other_level in levels:
            ...
            for i in range(len(assign_schedules_at_other_level)):
                assign_schedules_at_other_level[i] = insert_const_dim_in_range(assign_schedules_at_other_level[i], level+idx, const)
                
        compute_schedule = insert_const_dim_in_range(compute_schedule, level+idx, const)
