...
def get_op_dwconv2d(
    ic, oh, ow, kh, kw, stride, dilation, virtual_axis=True
    ):
    if virtual_axis:
        operator = BasicOperator(
            ...
            access_I = isl.BasicMap(f"{{ [v0, ic,oh,ow,kh,kw] -> I[ic, oh * {stride} + kh * {dilation}, ow * {stride} + kw * {dilation} ] }}"),
...
def get_op_conv1d(
    oc, ic, oh, ow, k, virtual_axis=True
    ):
    operator = BasicOperator(
        domain = isl.BasicSet(
