...
def transform_access(ori_access, n_inner_level):
    ...
    pwaff_access_sizes = utils.get_dynamic_shape_from_dynamic_map(outer_access) #[level:]
...
def vectorize_data_movement(data_movement):
    ...
    access_O_sizes = [data_movement.access_O.offsets.range().dim_max_val(i) for i in range(n_access_O_dim)]
    access_I_sizes = [data_movement.access_I.offsets.range().dim_max_val(i) for i in range(n_access_I_dim)]
